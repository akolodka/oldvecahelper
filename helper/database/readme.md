...
Содержимое кэш-файла загружается в память и представляется единой строкой. Поиск осуществляется на основании определения первого вхождения символов искомого запроса в строке кэш-файла (InStr).
...
После нажатия на кнопку «Принять» будет открыт файл элемента базы данных для заполнения:

![Title](https://github.com/akolodka/VBA/blob/main/resources/databaseNew_NewItemData.jpg) 

Корректировка данных выполняется напрямую через текстовый редактор без дополнительных форм.
При следующем запуске пользовательской формы будет произведено повторное кэширование:
